...
import com.ohgiraffers.parameterized.DateValidator;
import com.ohgiraffers.parameterized.NumberValidator;
import com.ohgiraffers.parameterized.StringValidator;
...
public class ParameterizedTests {
     ...
     *  parameterized-test 는 @ParameterizedTest 어노테이션을 @Test 어노테이션을 데체하여 사용하며,
    ...
    /* 필기.
     *  @ValueSource 를 이용하여 한 개의 파라미터로 전달할 값들의 목록을 지정할 수 있다.
     *  이 때 지원하는 타입은 다음과 같다.
     ...
     * 문자열이나 클래스 타입인 경우 null 이나 빈 값을 파라미터로 전달하기 위해 사용되는 어노테이션이다.
     * null 값과 빈 값을 모두 전달하기 위해 구성된 @NullAndEmptySource 도 이용할 수 있다.
     */
    ...
    @DisplayName("Month 에 정의된 타입들이 1~12월 사이의 범위인지 테스트")
    @ParameterizedTest
    ...
    /* 필기. Stream 을 반환하는 메소드를 만들어서 이를 테스트에 활용 할 수 있다. */
    ...
    /* 필기.
     *   JUnit5는 인수로 지정된 String 을 Enum 변환한다. 이처럼 기본적으로 제공하는 변환을 암시적 변환이라고 한다.
