...
public class EnvAnnotationTests {
    ...
    /* 수업목표. 운영체제, JRE에 따라 테스트를 수행 할 수있다. */
    /* 필기
    * 테스트 메소드는 특정 OS 환경에서만 메소드를 진행 되게 할 수 있다.
    * 이 때, 특정 OS에서 무시되는 사유를 disabledReason에 기술 하여
    * 테스트 코드가 무시되는 사유를 다른 사람도 함께 할 수 있게 한다.
     */
...
//
//    @Test
//    @DisabledOnOs(value = {OS.WINDOWS, OS.LINUX}, disabledReason = "윈도우와 리눅스만 테스트 합니다.")
...
//    }
    ...
    * @EnabledOnJreRange 를 이용하면 min 과 max 속성 사이의 버전에서 테스트 하는 것도 가능하다.
    * (min 만 작성시 min-최신버전까지. max만 작성시 이전 버전부터 max 까지만 테스트를 합니다.)
     */
