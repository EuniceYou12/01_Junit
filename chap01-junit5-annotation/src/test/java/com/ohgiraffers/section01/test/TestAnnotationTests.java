...
public class TestAnnotationTests {
     ...
     * 1. Junit Platform
     * -JVM 에서 테스트 프레임 워크를 실행 하기 위한 테스트 엔진을 제공한다.
     * 2. Junit Jupiter
     * -Junit5 에서 테스트를 작성하고 실행하기 위한 엔진을 제공한다.
     * 3. Junit Vintage
     * -Junit3, 4 를 기반으로 돌아가는 테스트 엔진을 제공해 준다. (하위 호환용)
     */

    /*
     * 테스트 클래스는 적어도 한개 이상의 @Test 어노테이션이 달린 메소드를 가진 클래스이다.
     * 테스트 클래스는 abstract 이면 안되고, 한개 이상의 생성자를 가지고 있어야한다.
     *
     * (두개 이상 작성하면 런타임시 PreconditionViolationException 이 발생 합니다.)
...
//        @DisplayName("DisplayName 우선순위 테스트")
        public void 테스트_메소드2() {
            /* 필기
            클래스 레벨에 DisplayNameGenerator 를 붙이게 되면 언더바를 공백으로 처리하여 테스트 이름을 바꿔 줄 수 있다.
